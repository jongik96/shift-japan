...
export default function EditPostPage() {
  ...
  useEffect(() => {
      ...
      if (mounted && editorRef.current && !editorInstance.current) {
        ...
        editorInstance.current.options.previewRender = (plainText: string) => {
          try {
            ...
            // Process standalone URLs and markdown links, convert them to preview cards
            // First, check if paragraph contains only a link (markdown link format)
            ...
            lines.forEach((line) => {
              ...
              const markdownLinkMatch = trimmedLine.match(markdownLinkPattern)
              if (markdownLinkMatch) {
                ...
                if (url.startsWith('http://') || url.startsWith('https://')) {
                  ...
                  // Escape special regex characters in URL and link text
                  const escapedUrl = url.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
                  const escapedText = linkText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
                  
                  // Match patterns that marked might create for markdown links
                  const patterns = [
                    new RegExp(`<p><a[^>]*href=["']${escapedUrl}["'][^>]*>${escapedText}<\\/a><\\/p>`, 'g'),
                    new RegExp(`<a[^>]*href=["']${escapedUrl}["'][^>]*>${escapedText}<\\/a>`, 'g'),
                  ]
                            ...
                            <div style="flex: 1; min-width: 0; padding: 1rem;">
                              <div style="display: flex; align-items: start; justify-content: space-between; gap: 0.5rem;">
                                <div style="flex: 1; min-width: 0;">
                                  <h4 class="link-preview-title" style="font-weight: 600; color: #111827; margin: 0 0 0.25rem 0; font-size: 1rem; line-height: 1.5;">${linkText}</h4>
                ...
                return // Skip URL pattern check if it's a markdown link
              }
        ...
        editorInstance.current.codemirror.on('change', () => {
          // Only update formData if change is from user input (not from external update)
          if (!isUpdatingFromExternalRef.current && editorInstance.current) {
            const value = editorInstance.current.value()
            // Use setTimeout to debounce and prevent focus issues
            setTimeout(() => {
              setFormData(prev => {
    ...
    // Only load editor when not loading (data is loaded)
    if (!loading) {
      loadEditor()
    }
  ...
  }, [loading]) // Only depend on loading, NOT formData.content

  // Set initial content when editor is ready and data is loaded (only once)
  useEffect(() => {
    if (editorInstance.current && formData.content && !loading && !initialContentSetRef.current) {
      ...
      if (!currentContent || currentContent.trim() === '') {
        ...
        setTimeout(() => {
          if (editorInstance.current && !initialContentSetRef.current) {
  ...
  }, [loading]) // Only run when loading changes to avoid re-triggering on every content change

  const fetchPost = async () => {
    try {
      // Try to find the post in any locale table
      let foundData = null
      let foundLocale: Locale = 'ja'
      
      for (const loc of locales) {
        const tableName = getTableName(loc)
      ...
      setTimeout(() => {
        if (editorInstance.current && !initialContentSetRef.current) {
          isUpdatingFromExternalRef.current = true
          editorInstance.current.value(markdown)
          isUpdatingFromExternalRef.current = false
          initialContentSetRef.current = true
        }
      }, 300)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.main_image && formData.main_image.trim()) {
        updateData.main_image = formData.main_image.trim()
      } else {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">✏️ 글 수정</h1>
        <p className="text-gray-600">글 내용을 수정하고 저장하세요</p>
      </div>
